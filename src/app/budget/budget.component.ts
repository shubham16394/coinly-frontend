...
export class BudgetComponent implements OnInit {
  ...
  getAllBudgetData() {
    const incomeObservable = this.budgetService.getBudget(this.email, this.month.toISOString(), 'income');
    const expenseObservable = this.budgetService.getBudget(this.email, this.month.toISOString(), 'expense');
    const savingObservable = this.budgetService.getBudget(this.email, this.month.toISOString(), 'saving');
  ...
  addData(data: any, type: type) {
    if(type === 'income') {
      ...
      const obj = {name: data?.name, value: data?.value, _id: data?._id, date: data?.createdAt, position: dataSourceData.length + 1};
      ...
      const obj = {name: data?.name, value: data?.value, _id: data?._id, date: data?.createdAt, position: expenseDataSourceData.length + 1};
      ...
      const obj = {name: data?.name, value: data?.value, _id: data?._id, date: data?.createdAt, position: savingsDataSourceData.length + 1};
  ...
  editData(data: any, type: type) {
    if(type === 'income') {
      const dataSourceData = this.dataSource?.data;
      const index = dataSourceData.findIndex((e: any) => {return e?._id === data?._id});
      if(index !== -1) {
  ...
  sortData(data: any) {
    return data.sort((a: any,b: any) => {return new Date(a?.position).getTime() - new Date(b?.position).getTime()});
  }

  totalValue(type: type) {
    if(type === 'income') {
      const data = this.dataSource.data;
      this.income = data.reduce((acc: any, curr: any) => { return acc + curr?.value; }, 0);  
    }
    else if(type === 'expense') {
      const data = this.expenseDataSource.data;
      this.expense = data.reduce((acc: any, curr: any) => { return acc + curr?.value; }, 0);
    }
    else if(type === 'saving') {
      const data = this.savingsDataSource.data;
      this.saving = data.reduce((acc: any, curr: any) => { return acc + curr?.value; }, 0);  
    }
  }
  ...
  edit(element: any, type: type) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if(diff) {
        this.budgetService.updateBudget(element?._id, {updateBudgetData: diff}).subscribe((res: any) => {
          if(res?.data) {
            this.editData(res?.data, type);
          }
          else {
            console.log('Empty edit response');
          }
        });
      }
      else {
  ...
  delete(element: any, type: type) {
    if(element?._id){
      this.budgetService.deleteBudget(element?._id).subscribe((res: any) => {
        if(res?.data?.value) {
